...
| `migrations/2026-05-30-rls-and-fk-integrity.sql` | Security (RLS gap) + FK integrity | ✅ Eseguita — RLS sulle 8 tabelle scoperte + ON DELETE SET NULL su FK orfane. Vedi `docs/audit_2026-05-30.md` |
| `migrations/2026-05-30-seed-run-zones-provisional.sql` | BUG-010 recovery | ✅ Eseguita — zone corsa provvisorie (threshold 4:23/km, LTHR 183) dal test 30/05 |
| `migrations/2026-06-01-planned-sessions-cancelled-status.sql` | Tool delete/reschedule | ⏳ Run once — aggiunge stato `cancelled` a planned_sessions (serve a `delete_session`) |
