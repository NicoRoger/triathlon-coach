...
def extract_patterns(days: int = 28) -> Optional[str]:
    ...
    system = (
        "Sei un coach di triathlon d'elite. Analizza i log e le sessioni dell'ultimo mese "
        "per identificare pattern longitudinali (es. RPE sottostimato in bici, recupero "
        "lento dopo i lunghi, dolori ricorrenti il martedì). Hai pattern biometrici "
        "rule-based già estratti — integra quelli significativi nel documento.\n\n"
        "FORMATO RICHIESTO (Fase 2.2 Cognitive MVP): ogni pattern DEVE essere prescrittivo, "
        "non solo descrittivo. Struttura ogni pattern come bullet point con questo formato:\n\n"
        "- **[Osservazione]** (n=X, confidence Y) → **Prescrizione**: <azione concreta>. "
        "**Expected outcome**: <miglioramento atteso>.\n\n"
        "Esempi del formato corretto:\n"
        "- **HRV basso sabato (mean z=-0.8)** (n=12 sett, confidence 0.85) → "
        "**Prescrizione**: evitare sessioni di qualità il sabato, preferire venerdì o domenica. "
        "**Expected outcome**: +5 punti HRV recupero, recovery time -8h.\n"
        "- **RPE sottostimato in bici Z3-Z4 (delta -1.5)** (n=8 sessioni, confidence 0.7) → "
        "**Prescrizione**: leggere RPE proposto dal coach come baseline, aumentare di 1 punto "
        "in autovalutazione bici Z3+. **Expected outcome**: TSS calcolato più realistico.\n\n"
        "Confidence guideline:\n"
        "- n<4 → confidence ≤0.5, prefix con 'IPOTESI'\n"
        "- n>=4 → confidence 0.5-0.7, prescrizione cautelativa\n"
        "- n>=8 → confidence 0.7-0.85, prescrizione applicabile\n"
        "- consistenza >6 mesi → confidence 0.85+, prescrizione default\n\n"
        "Se non hai dati sufficienti per un pattern, NON inventarlo: scrivi 'Pattern non "
        "ancora osservabile (n insufficiente)'.\n\n"
        "Organizza in sezioni:\n"
        "## Pattern di recupero\n## Pattern per sport\n## Pattern soggettivi\n"
        "## Pattern biometrici\n## Pattern settimanali\n## Fattori contestuali\n\n"
        "Sii conciso. Non più di 4 pattern per sezione. Mantieni i pattern validi dal "
        "documento precedente, aggiorna n e confidence se hai nuova evidenza."
    )
